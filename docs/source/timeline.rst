Timeline
===================

.. admonition:: 16th November 2025

   The Chinese Society of Information Fusion (CSIF) hosted a workshop at Shanghai Jiao Tong University.

   This event brought together leading experts from academia and industry to exploreInformation Fusion technologies for autonomous systems and strengthen international collaboration.
   The Chinese Society of Information Fusion (CSIF) hosted an impactful workshop at Shanghai Jiao Tong University. 
  
    .. _CSIF-WorkShop-2025:

    .. figure:: figures/CSIF_Workshop_2025.jpg
        :align: center
        :alt: CSIF Workshop on Information Fusion for Autonomous Systems, Nov.16. 2025
        
        Fig. 01: CSIF Workshop on Information Fusion for Autonomous Systems, Nov.16. 2025


.. admonition:: May 2025

    We initiated the development of the **Smart Data Center**, the foundational software infrastructure for FOOD. 
    
    This platform aims to provide a reliable and high-performance storage architecture. It features advanced conversion tools designed to transform multi-source heterogeneous sensor data into a **unified data structure**, ensuring efficient organization and retrieval.
   


.. admonition:: 28th July 2024

   We did a real-world operational test of the vehicle inside SJTU:
   
   #. Selection of intersections of collection significance;
   #. Tests device stability over long periods of time, over long distances and at high temperatures;
   #. Training on safety precautions during data collection.

   .. figure:: figures/group_photo.jpg
      :align: center
      :alt: group-photo

      Fig. 02: Recent Group 

   .. admonition:: Related Videos

      .. raw:: html

         <div style="text-align: center;">
            <video width="600" controls>
               <source src="_static/videos/car_running.mp4" type="video/mp4">
               Your browser does not support the video tag.
            </video>
            <div style="text-align: center;">
            Fig. 03: Vehicle display
         </div>

.. admonition:: 22nd July 2024

   We assemble vehicle components:
   
   #. Lidar camera millimetre wave radar assembly test.
   #. Range test of on-board batteries.

   .. figure:: figures/History_assemble.jpg
      :align: center
      :alt: History_assemble

      Fig. 04: Vehicle Assembly


